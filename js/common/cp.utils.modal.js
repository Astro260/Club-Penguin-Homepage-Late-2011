...
CP.utils.Modal.prototype.open = function(modalContent, openCallback, closeCallback){
	...
	if(!this.isOpen){
		...
		this.modal.css({visibility: 'hidden'}); ////web-beta.archive.org/web/20120428172738/http://to support interapp linking, instead of //this.modal.hide();
		this.modalContent.empty();
		this.modalContent.append(modalContent);
		this.options.onOpenStart();
		this.modal.css({visibility: 'visible'}); //web-beta.archive.org/web/20120428172738/http://to support interapp linking, instead of //this.modal.show();
		this.centerElement(this.modal);
		// update state
		this.isOpen = true;
		this.isLoading = false;
...
CP.utils.Modal.prototype.close = function(callback){
	if(this.isOpen){
		...
		//web-beta.archive.org/web/20120428172738/http://If you are in IE, find all embedded objects and replace them with divs. fadeout doesn't play too nice them
...
/*
     FILE ARCHIVED ON 17:27:38 Apr 28, 2012 AND RETRIEVED FROM THE
